...
#ifndef _INCLUDED_UI_SELECT_IMAGE_BOX_H_
#define _INCLUDED_UI_SELECT_IMAGE_OX_H_
...
class UiSelectImageBox : public toybox::TbUiWidget
{
public:

    typedef toybox::TbUiWidget Super;
    typedef UiSelectImageBox Self;
...
public:
    ...
    // デストラクタ
    virtual ~UiSelectImageBox();

public:

    // 画像サイズ設定
    void SetImageSize( const TbDimensionF32& imageSize  ){
        m_ImageBox.SetSize(imageSize.width,imageSize.height);
    }

    // アイテム追加
    void AddItem( const TbRectF32& uv );
    ...
    void SetSelectedIndex( TbSint32 index ) {
        if( 0 <= index && index < GetItemCount() ){
            m_Index = index;
            m_ImageBox.SetImage(nullptr,m_Items[index].uv);
        }
    }
