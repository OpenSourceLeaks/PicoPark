...
TB_STATIC_ASSERT(TB_ARRAY_COUNT_OF(MAP_CHIP_ATTR)==PUZZLE_MAP_CHIP_MAX-CR_TILE_MAP_CHIP_USER_BEGIN);

static TbFloat32 UV_ERROR = 0.0001f;
#if !CMN_ENABLE_BUILD_100P
#define PUZ_TEX_UV_RECT(x,y,w,h) CMN_TEX_UV_RECT_2(x+1,y+1,w,h,UV_ERROR)
#else // 100人テトリス用
...
static TB_NDEBUG_CONST TbRectF32 MAP_CHIP_UV_RECT[] = 
{
    ...
    TbRectF32() , // CR_TILE_MAP_CHIP_NONE , 
...
void PuzzleMap::OnDrawBack( const TbMatrix& poseMatrix , TbSpriteMaterial* material )
{
    ...
    for( TbUint32 y = 0; y < height; ++y ) {
        for( TbUint32 x = 0; x < width; ++x ) {
            ...
            if( chip >= CR_TILE_MAP_CHIP_BLOCK )
            {
