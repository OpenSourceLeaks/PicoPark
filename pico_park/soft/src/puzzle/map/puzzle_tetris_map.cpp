...
static TB_NDEBUG_CONST TbFloat32 REPEAT_FIRST_SEC = 134.0f;
    ...
    , m_CountUpLineCount(0)
{
    ...
    SetFallFloorTime(FALL_TIME_NORMAL);
...
PuzzleTetrisMap::~PuzzleTetrisMap()
{
    TbInputSetting::SetRepeatMilliSecFirst(Root::REPEAT_FIRST_SEC);
}
...
void PuzzleTetrisMap::OnInitialize( TbScriptLuaNode* node )
{
    ...
    if (node && TbScriptLuaNode::RESULT_OK == node->TryToFloat32(fallTimeDefault, "fallTimeFloorDefault")) {
        SetFallFloorTime(fallTimeDefault);
    }
    ...
    if( node && TbScriptLuaNode::RESULT_OK == node->GoToChild("fallTimeTable") ) 
    {
        ...
        if( TbScriptLuaNode::RESULT_OK == node->TryToTableLength(count) ){
            ...
            for( TbUint32 i = 0; i < count; ++i ) {
                ...
                TbFloat32 time = 1.0f;
                TbUint32 index = Root::GetInstance().GetPlayerCount() - CMN_PLAYER_MIN;
                node->TryToFloat32ByIndex(time,1+index);
        ...
        if( lineCount > 0 ) {
            do {
                ...
                for( TbSint32 copyX = judgeRect.x; copyX < endX; ++copyX ){
                    ...
                    if( srcChip != CR_TILE_MAP_CHIP_NONE && srcChip < PUZZLE_MAP_CHIP_PRE_BLOCK ){
                        isEnableCopy = TB_FALSE;
                    }
                    if( dstChip != CR_TILE_MAP_CHIP_NONE && dstChip < PUZZLE_MAP_CHIP_PRE_BLOCK  ){
                        continue;
                    }
                    if( isEnableCopy ) {
                        SetMapChip(copyX,y,srcChip);
                        SetMapChip(copyX,y-lineCount, CR_TILE_MAP_CHIP_NONE);
                    }else{
                        SetMapChip(copyX,y, CR_TILE_MAP_CHIP_NONE);
                    }
                }
