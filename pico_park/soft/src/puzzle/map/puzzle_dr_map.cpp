...
void PuzzleDrMap::updateDr()
{
    ...
    {
        TbUint32 count = 0;
        TbUint32 currentChip = CR_TILE_MAP_CHIP_NONE;
            ...
            if( isValid ){
                ...
                if( count >= 4 ){
                    ...
                    for (TbUint32 eraseX = 0; eraseX < count; ++eraseX){
                        SetMapChip(x-1-eraseX,y,CR_TILE_MAP_CHIP_NONE);
                    }
                    isErased = TB_TRUE;
                }
                if( isActive ){
                    ...
                    count = 0;
                    currentChip = CR_TILE_MAP_CHIP_NONE;
                }
            }
        }
        if( count >= 4 ){
            // ボーダー超えていたので消す
            for (TbUint32 eraseX = 0; eraseX < count; ++eraseX){
                SetMapChip(endX-1-eraseX,y,CR_TILE_MAP_CHIP_NONE);
            }
            isErased = TB_TRUE;
        }else{
            count = 0;
    ...
    for (TbSint32 x = judgeRect.x; x < endX; ++x)
    {
            ...
            if( isValid ){
                ...
                if( count >= 4 ){
                    ...
                    for (TbUint32 eraseY = 0; eraseY < count; ++eraseY){
                        SetMapChip(x,y-1-eraseY,CR_TILE_MAP_CHIP_NONE);
                    }
                    isErased = TB_TRUE;
                }
                if( isActive ){
        ...
        if( count >= 4 ){
            // ボーダー超えていたので消す
            for (TbUint32 eraseY = 0; eraseY < count; ++eraseY){
                SetMapChip(x,endY-1-eraseY,CR_TILE_MAP_CHIP_NONE);
            }
            isErased = TB_TRUE;
        }else{
            count = 0;
