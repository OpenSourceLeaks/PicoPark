...
void BreakoutBar::OnUpdateAction( TbFloat32 deltatime )
{
    ...
    // 上下
    SetVeloY(0.0f);
    if ( isOnInput(CrInput::BUTTON_UP) ){
        SetVeloY(-SPEED);
    }else if ( isOnInput(CrInput::BUTTON_DOWN) ){
        SetVeloY(SPEED);
    }

    // 左右
    SetVeloX(0.0f);
    if ( isOnInput(CrInput::BUTTON_LEFT) ){
        SetVeloX(-SPEED);
    }else if ( isOnInput(CrInput::BUTTON_RIGHT) ){
        SetVeloX(SPEED);
    }
 ...
 * @since 2014.07.27
 */
TbBool BreakoutBar::isOnInput( CrInput::Button button ) const
{
    TbBool result = TB_FALSE;
    result = CrInput::IsOn(button,m_PlayerIndex);
    return result;
}
 ...
 * @since 2014.07.27
 */
TbBool BreakoutBar::isPressInput( CrInput::Button button ) const
{
    TbBool result = TB_FALSE;
    result = CrInput::IsPress(button,m_PlayerIndex);
    return result;
}
 ...
 * @since 2014.07.27
 */
TbBool BreakoutBar::isReleaseInput( CrInput::Button button ) const
{
    TbBool result = TB_FALSE;
    result = CrInput::IsRelease(button,m_PlayerIndex);
    return result;
}
