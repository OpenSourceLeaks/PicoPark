...
void SysMajorityInput::Update( TbFloat32 deltatime )
{
    ...
    for( TbSint32 i = 0; i < CrInput::BUTTON_MAX; ++i ){
        update( static_cast<CrInput::Button>(i) , deltatime );
    }
}
 ...
 * @since 2013.10.26
 */
TbBool SysMajorityInput::IsOn( CrInput::Button button  ) const
{
 ...
 * @since 2013.10.26
 */
TbBool SysMajorityInput::IsPress( CrInput::Button button ) const
{
 ...
 * @since 2013.10.26
 */
TbBool SysMajorityInput::IsRelease( CrInput::Button button ) const
{
    return (!m_Now.Test(button) & m_Old.Test(button));
}
 ...
 * @since 2013.10.26
 */
void SysMajorityInput::update( CrInput::Button button , TbFloat32 deltatime )
{
    TbUint32 count = 0;
    for( TbUint32 i = 0; i < m_InputCount; ++i ) {
        if( CrInput::IsOn(button,i) ){
#if 0 // 有効時間制は評判悪いので廃止
