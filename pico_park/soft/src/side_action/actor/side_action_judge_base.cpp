        ...
        {
            {
                if( m_IsEven ){
                    ...
                    if (!m_ResultMessage.IsEmpty()) {
                        m_MessageWindow.SetMessage(m_ResultMessage.GetBuffer());
                    }else{
                ...
                m_MessageWindow.ReserverClose(5.0f);
                setPhase(m_Phase+1);
            }
        }
        break;
    ...
    case PHASE_RESULT + 2:
        {
            ...
            if (m_IsEven) {
                BattleManager::GetInstance().SetResult(BattleManager::RESULT_DRAW);
                SeqManager::GetInstance().RequestSequence(SEQ_BATTLE_SELECT, GetActionStage()->GetId());
            }
            else {
                BattleManager::GetInstance().SetResult(m_PrizePlayerIndex);
                SeqManager::GetInstance().RequestSequence(SEQ_BATTLE_RESULT, GetActionStage()->GetId());
            }
        }
        break;
    }
...
TbSint32 SideActionJudgeBase::NotifyEvent( TbUint32 event , void* param )
{
    ...
    static TbFloat32 FADE_OUT_SEC = 3.5f;
    // プレイヤーの動きを止める
    TbPlaySound("whistle",2.0f);
    SeqManager::GetInstance().FadeOutBgm(FADE_OUT_SEC);
    ...
    case PHASE_TIMEUP:
        {
            ...
            // 数
            fontBuffer->Printf(m_FinishMessageX, stage->GetWindowHeight()/2,m_FinishMessage.GetBuffer());
