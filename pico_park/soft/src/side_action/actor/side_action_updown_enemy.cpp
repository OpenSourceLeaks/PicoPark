...
{
    SPRITE_RECT.x+2 , SPRITE_RECT.y+4 , SPRITE_RECT.width-4 , SPRITE_RECT.height-4
};
 ...
 * @since 2013.10.16
 */
void SideActionUpDownEnemy::onCollided(CrActionCollision* target)
{
    if( target->GetLayer() == SIDE_ACTION_COLLISION_LAYER_PLAYER ){
        CrActionActor* actor = static_cast<CrActionActor*>(target->GetActor());
        if( actor ) {
            TbPlaySound("hit");
