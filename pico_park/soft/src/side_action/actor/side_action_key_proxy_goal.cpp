    ...
    : Super()
{
    ...
    {
        pushBackTarget = CrActionCollision::PUSH_BACK_TARGET_COLLISION;
        CrActionCollision* collision = CreateCollision( PUSHBACK_COLLISION_RECT[i] , pushBackTarget , TB_TRUE );
        if( TB_VERIFY(collision)) {
 ...
 * @author teco
 */
void SideActionKeyProxyGoal::onCollided(CrActionCollision* target)
{
