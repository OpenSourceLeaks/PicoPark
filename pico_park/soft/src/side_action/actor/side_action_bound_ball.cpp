...
void SideActionBoundBall::OnUpdateAction( TbFloat32 deltatime )
{
    if( m_DeadTime > 0 ) {
        ...
        if( GetCollision()->IsContacted(CR_CONTACT_DOWN) )
        {
            // 左右の速度を摩擦
 ...
 * @since 2014.07.27
 */
void SideActionBoundBall::onContacted(const TbVector2& normal,CrActionCollision::ContactTarget target,const CrActionCollisionInfo& info)
{
    // コリジョン同士の衝突
    ...
    if( info.collision && info.collision->GetActor() ) {
        ...
        if( 0 == m_DeadTime ){
            if( ( info.collision->GetLayer() == SIDE_ACTION_COLLISION_LAYER_PLAYER ||
                info.collision->GetLayer()   == SIDE_ACTION_COLLISION_LAYER_OBJECT)
                &&
