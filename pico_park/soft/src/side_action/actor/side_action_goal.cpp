 ...
 * @author teco
 */
void SideActionGoal::Setup(const CrActorFactory::UserData& userData)
{
    TbUint32 pushBackTarget = 0;
    TbRectF32 rect = COLLISION_RECT;
    if ( 0 < userData.GetCount())
    {
        TbFloat32 offset = userData.AsFloat32(0);
        rect.y -= offset;
        rect.height += offset;
    }
 ...
 * @author teco
 */
void SideActionGoal::OnEnter(void* userData)
{
    Super::OnEnter(userData);
    SideActionStage* stage = TbDynamicCast<SideActionStage*>(GetStage());
    AddPos(stage->GetAutoScrollEndOffset(),0.0f,0.0f);
}
 ...
 * @since 2013.10.16
 */
void SideActionGoal::onCollided(CrActionCollision* target)
{
