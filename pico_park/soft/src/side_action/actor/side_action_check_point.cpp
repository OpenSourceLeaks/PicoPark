...
void SideActionCheckPoint::OnUpdateAction( TbFloat32 deltatime )
{
    ...
    if( m_BitArray.Test(FLAG_CHECK) )
    {
        ...
        font->PushSetting();
        font->SetFontSize(FONT_SIZE * scale);
        font->SetAlignX(TbFontBuffer2d::ALIGN_X_CENTER);
        font->SetAlignY(TbFontBuffer2d::ALIGN_Y_DOWN);
        Root::GetInstance().GetFont()->Printf(static_cast<TbSint32>((GetPosX()- GetStage()->GetScrollX())*scale),
                                              static_cast<TbSint32>((GetPosY()-rect.height*HEIGHT_SCALE)*scale),
                                              "CHECK");
        font->PopSetting();
    }
}
 ...
 * @since 2013.10.16
 */
void SideActionCheckPoint::onCollided(CrActionCollision* target)
{
    ...
    case SIDE_ACTION_COLLISION_LAYER_PLAYER:
        {
            if( !m_BitArray.Test(FLAG_CHECK) ){
                ...
                globalData.SetCheckPoint(stage->GetId(),pos,stage->GetPlaySec());
