...
static TB_NDEBUG_CONST TbFloat32 RETURN_SPEED           = 0.75f;
    ...
    , m_PushBackCount(0)
{
    ...
    // 移動開始センサー
    {
        ...
        {
            SPRITE_RECT.x+SPRITE_RECT.width+1.0f , 
            SPRITE_RECT.y+1 , 
            triggerWidth , 
            SPRITE_RECT.height-2.0f 
        };
        ActionCollision* triggerCollision = CreateCollision( RAY_COLLISION_RECT , 0 , TB_TRUE );
...
void SideActionMoveWall::OnUpdateAction( TbFloat32 deltatime )
{
    ...
    if( IsEnableMoveContactActor(mov,CMN_CONTACT_UP) ){
        ...
        if( movX > 0.0f ){
            MoveContactActor( mov , CMN_CONTACT_RIGHT , option );
        }else{
            MoveContactActor( mov , CMN_CONTACT_LEFT , option);
        }
    }else{
 ...
 * @since 2014.09.11
 */
void SideActionMoveWall::onCollidedCollision(ActionCollision* collision)
{
