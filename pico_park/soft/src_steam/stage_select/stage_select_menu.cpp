    ...
    , m_StageIndex(initIndex)
{
    ...
    for( TbSint32 i = 0; i < pageCount; ++i ){
        ...
        for( TbSint32 b = 0; b < itemCount; ++b ) 
        {
            ...
            // テクスチャ
            const char* stageName = SeqManager::GetInstance().GetStageName(stageIndex);
            TbStaticString64 path = TbStaticString64::Format("tex/tex_%s.tga",stageName);
...
void StageSelectMenu::OnUpdate( TbFloat32 deltaSec )
{
    ...
    case PHASE_IDLE:
        {
            // フォーカス切り替え
            if( SysInput::IsAnyoneOnRepeat(SysInput::BUTTON_LEFT,GetPlayerBit()) ){
                addItemIndex(TB_FALSE);
            }else if( SysInput::IsAnyoneOnRepeat(SysInput::BUTTON_RIGHT,GetPlayerBit()) ){
...
void StageSelectMenu::onFocusChangedStage( TbSint32 stageIndex )
{
    ...
    if( item ){
        ...
        case CMN_STAGE_TYPE_BEST_TIME:
            {
                TbUint32 bestTime = Root::GetInstance().GetGlobalData().GetBestMillSec(stageIndex,playerCount);
                if( StageData::TIME_INVALID == bestTime  ){
