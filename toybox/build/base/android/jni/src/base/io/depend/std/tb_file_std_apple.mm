...
#import "base/io/depend/std/tb_file_std.h"
#import "base/io/tb_file_util.h"
#import "base/math/tb_math_util.h"
...
namespace
{
    ...
    // ハンドルを開く
    FILE* openHandle( const char* path , const char* option )
    {
        NSString* pathStr = [ [ NSString alloc ] initWithUTF8String:path ];
        NSString* fileName = [pathStr lastPathComponent]; //=> @"test.txt"
        NSString* baseName = [fileName stringByDeletingPathExtension]; //=> @"c"
        NSString* extension = [fileName pathExtension]; //=> @"c"
        NSString* bundlePath = [[NSBundle mainBundle] pathForResource:baseName ofType:extension];
        FILE* fp = fopen([bundlePath cStringUsingEncoding:1],option);
        return fp;
    }
}
...
TbUint32 TbFileStd::GetFileSize( const char* path , TbFilePathId pathId )
{
    ...
    FILE* fp = openHandle( fullpath.GetBuffer() , "rb" );
    fseek( fp , 0 , SEEK_END );
    if( !fp ){
        return 0;
    }
...
TbResult TbFileStd::Open()
{
    ...
    m_Handle = openHandle( GetPath() , mode );
