...
namespace
{
    ...
    const char VERTEX_SHADER_CODE[] =
        ...
        "void main()                                             \n"
        "{                                                       \n"
        "    gl_Position =  u_ModelViewProjMatrix * a_Position0; \n"
    ...
    const char PIXEL_SHADER_CODE[] =
        ...
        "varying   vec4 v_Texcoord0;                             \n"
        "void main() {                                           \n"
        "    gl_FragColor = v_FrontColor * texture2D( u_Texture0 , v_Texcoord0.st );  \n"
        "}                                                       \n";
#else
    // ピクセルシェーダ用コード
    const char PIXEL_SHADER_CODE[] =
...
TbBool TbFixedShaderDepend::EnableVertexAttribute( const void* vertexs , TbUint32 stride )
{
    ...
    // シェーダ側へ送る
    TbRenderDevice& device = TbRenderDevice::GetInstance();
    return device.GetShaderManager().GetDepend().EnableVertexAttribute( vertexs , stride );
}
...
TbBool TbFixedShaderDepend::DisableVertexAttribute()
{
    // シェーダ側へ送る
    TbRenderDevice& device = TbRenderDevice::GetInstance();
    return device.GetShaderManager().GetDepend().DisableVertexAttribute();
}
