...
void TbPadDepend::Update()
{
    ...
    for (TbUint32 i = 0; i < TB_ARRAY_COUNT_OF(m_PadInfo); ++i)
    {
        ...
        if (style.Test<nn::hid::NpadStyleJoyLeft>())
        {
            ...
            nn::hid::GetNpadState(&state, NPAD_ID[i]);
        ...
        else if (style.Test<nn::hid::NpadStyleJoyDual>())
        {
            ...
            if (state.buttons.Test<nn::hid::NpadButton::L>())
            {
                padInfo.now |= TB_PAD_L1_BIT;
            }
            if (state.buttons.Test<nn::hid::NpadButton::R>())
            {
                padInfo.now |= TB_PAD_R1_BIT;
            }
        }
...
void TbPadDepend::setup()
{
    ...
    // 使用する Npad を設定
    nn::hid::SetSupportedNpadIdType(NPAD_ID, TB_ARRAY_COUNT_OF(NPAD_ID));
}
