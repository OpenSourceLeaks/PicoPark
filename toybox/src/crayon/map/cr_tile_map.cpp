...
TB_STATIC_ASSERT(TB_ARRAY_COUNT_OF(MAP_CHIP_DEFAULT_ATTR)==CR_TILE_MAP_CHIP_USER_BEGIN);

static const TbUint32 BLK = CR_TILE_MAP_CHIP_BLOCK;
static const TbUint32 NON = CR_TILE_MAP_CHIP_NONE;

static const TbUint32 TEST_WIDTH = 16;
static const TbUint32 TEST_SIZE = 32;
static CrTileMapChip TEST_STAGE[] = 
{
...
CrTileMap::CrTileMap()
    ...
    , m_Height(0)
    , m_ChipSize(0)
    , m_CullingRect(0,0,TB_UINT32_MAX,TB_UINT32_MAX)
    , m_PlayerCount(0)
    , m_MapData(NULL)
    ...
    , m_Scroll(0.0f)
{
    ...
    m_SpriteHookerBack.SetZ(CR_SPRITE_Z_BACK_2);
    m_SpriteHookerFront.SetDrawFunc(TbCreateMemFunc(this,&Self::OnDrawFront));
    m_SpriteHookerFront.SetZ(CR_SPRITE_Z_FRONT_3);
 ...
 * @since 2013.09.11
 */
void CrTileMap::Initialize( TbUint32 width , TbUint32 height , TbUint32 chipSize , TbUint32 playerCount )
{
    release();
    m_Width = width;
    m_Height = height;
    ...
    m_MapData = new CrTileMapChip[m_Height*m_Width];
 ...
 * @since 2013.09.28
 */
void CrTileMap::Initialize( TbScriptLuaNode* node , TbUint32 playerCount)
{
    if( node )
    {
        m_PlayerCount = playerCount;
        ...
        // 可変テーブル参照
        if( variable ){
            m_BitArray.SetBit(FLAG_VARIABLE,TB_TRUE);
 ...
 * @since 2013.09.15
 */
TbBool CrTileMap::IsMovable( TbSint32 x , TbSint32 y ) const
{
    TbSint32 mcX = TbClamp<TbSint32>(x,0,m_Width-1);
    TbSint32 mcY = TbClamp<TbSint32>(y,0,m_Height-1);
    return !(GetMapChipAttr(mcX,mcY) & CR_TILE_MAP_CHIP_ATTR_COLLISION);
}
 ...
 * @since 2013.10.13
 */
TbBool CrTileMap::IsMovable( TbPointS32& from , TbPointS32& to , TbPointS32* contactPoint ) const
{
 ...
 * @since 2013.09.15
 */
TbBool CrTileMap::IsMovableX( TbPointS32& point , TbSint32 height , TbPointS32* contactPoint ) const
{
 ...
 * @since 2013.09.14
 */
TbBool CrTileMap::IsMovableY( TbPointS32& point , TbSint32 width , TbPointS32* contactPoint ) const
{
 ...
 * @since 2013.10.24
 */
void CrTileMap::EnterSpriteScene( TbSpriteScene* scene )
{
 ...
 * @since 2013.10.24
 */
void CrTileMap::ExitSpriteScene()
{
 ...
 * @since 2013.10.24
 */
void CrTileMap::OnDrawBack( const TbMatrix& poseMatrix , TbSpriteMaterial* material )
{
    // デバッグ表示
    for( TbUint32 y = 0; y < m_Height; ++y ) {
        for( TbUint32 x = 0; x < m_Width; ++x ) {
            if( m_MapData[y*m_Width+x].type == BLK && !IsCulling(x,y) )
            {
                TbRectF32 rect( static_cast<TbFloat32>((x)*m_ChipSize) - GetScroll() , 
                                static_cast<TbFloat32>((y)*m_ChipSize) , 
 ...
 * @since 2013.10.24
 */
TbUint32 CrTileMap::GetMapChipAttr( TbUint32 x , TbUint32 y ) const
{
    TbUint32 type = GetMapChip(x,y);
    if( type < CR_TILE_MAP_CHIP_USER_BEGIN ){
        return MAP_CHIP_DEFAULT_ATTR[type];
    }
    TbUint32 userType = (type-CR_TILE_MAP_CHIP_USER_BEGIN);
